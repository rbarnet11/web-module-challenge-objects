...
console.log(createMenuItem('pies', 10, 'dessert'));
...
*/
 ...
 function getReviewByRating(array, rating) {
  ...
  for (let i = 0; i < array.length; i++ ) {
    let ratingVal = array[i].rating;
    // console.log(ratingVal);
    if ((ratingVal >= Math.floor(rating)) && (ratingVal < Math.floor(rating+1))) {
      reviewByRating.push(array[i]);
    }
  }
  return reviewByRating;
 }
 // getReviewByRating(reviews, 4.9);
...
function getLongReviews(array) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        feedbackArray.push(words);
    }
    let longReviews = feedbackArray.filter(wordArr => wordArr.length > 15);
...
function carMaker(miles) {
  const car = {
    ...
    drive: function(distance) {
      let newDistance = this.odometer += distance;
      return newDistance;
    }
  }
  return car;
